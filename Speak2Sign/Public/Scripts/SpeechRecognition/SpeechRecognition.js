...
//@ui {"widget":"group_end"}
var handHint = script.handSignHintController.api;
...
var onUpdateListeningEventHandler = function(eventArgs) {
    ...
    if (script.transcripitonText && (eventArgs.isFinalTranscription || eventArgs.transcription.trim())) {        
        ...
        signWord(script.handSignHintController.api, "hello");
    }
            ...
            for (var kIterator=0; kIterator<keywords.length; kIterator++) {
                ...
                if (doesKeywordExist(keyword) && (script.transcriptionType === TranscriptionType.LIVE_TRANSCRIPTION || eventArgs.isFinalTranscription) && prevKeyword !== keyword) {
                    ...
                    if (script.keywordText) {
                        script.screenTextKeywordResponses.text = keywordResponseText!=="" ? ("Sign: "+ keywordResponseText) : "";
                        signWord(script.handSignHintController.api, keywordResponseText);                    
                    }
...
function showLetter(chr)
{
    handHint.showLetter(chr);
};
function spellDog()
{
    showLetter("D"); 
    ...
    delayedEvent2.reset(2.5);
};

function signWord(handHint, text){

    var dog = "DOG";
    if(text.toString === dog.toString)
    {
        spellDog();
    }    
}
